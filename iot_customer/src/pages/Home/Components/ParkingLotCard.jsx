...
const ParkingLotCard = ({ lot }) => {
  const { total_spaces, available_spaces, occupied_spaces, vehicle_type_id } =
    lot;
  ...
  const types = useVehicleTypeStore((state) => state.vehicleTypes);
  const price =
    types.find((type) => type.vehicle_type_id === vehicle_type_id)
      ?.fee_per_hour || 0;
      ...
      <div className="flex justify-between">
        <div className="flex items-center gap-2 text-gray-500 text-sm mb-1">
          ...
          <span className="font-medium">
            {getTypeNameById(vehicle_type_id)}
          </span>
        </div>
        <span>
          {price}$ <span className="text-gray-400">per hour</span>
        </span>
      </div>
