...
const ParkingSpaceForm = ({ open, onClose, onSave, initialData, mode }) => {
    ...
    const handleChange = (e) => {
        ...
        setFormData((prev) => ({ ...prev, [name]: name === "total_spaces" ? Number.parseInt(value) || "" : value }))
    ...
    return (
        <Dialog open={open} onClose={() => !loading && onClose()} maxWidth="sm" fullWidth>
            <DialogTitle>{mode === "create" ? "Add New Parking Space" : "Edit Parking Space"}</DialogTitle>

            <DialogContent dividers>
                {mode === "create" && (
                    <FormControl fullWidth margin="normal" error={!!errors.vehicle_type_id}>
                        ...
                        {errors.vehicle_type_id && <FormHelperText>{errors.vehicle_type_id}</FormHelperText>}
                    </FormControl>
                )}
                ...
                {mode === "edit" && (
                    <Alert severity="info" sx={{ mt: 2 }}>
                        Note: Reducing total spaces below the current occupied count may cause issues.
                    </Alert>
                )}
            </DialogContent>
            ...
            <DialogActions sx={{ px: 3, py: 2 }}>
                ...
                <Button
                    ...
                    disabled={loading}
                    startIcon={loading ? <Loader className="animate-spin" size={18} /> : <Check size={18} />}
                >
                    {loading ? "Saving..." : "Save"}
                </Button>
            </DialogActions>
