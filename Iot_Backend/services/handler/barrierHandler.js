...
async function barrierHandler(client, topic,data) {
    ...
    const card = await RfidCard.findOne({ where: { card_number }, include: { model: User } });
    if (!card || !card.user) {
        mqttEventEmitter.emit('scan', { status: "fail",embed_id, action, message:  `Card or user not found`  });
        return client.publish(`${topic}/response/${embed_id}`, JSON.stringify({ status: 'invalid', message: 'Card or user not found' }));
    ...
    if (action === 'enter' && card.status === 'exited') {
        ...
        if (!session) {
            mqttEventEmitter.emit('scan', { status: "fail",embed_id, card_number,vehicle_number:card.vehicle_number, action, message:  `Invalid ${logAction} status`  });
            return client.publish(`${topic}/response/${embed_id}`, JSON.stringify({ success: false, message: `Invalid ${logAction} status` }));
        }
        ...
        mqttEventEmitter.emit('scan', { success: false,embed_id, card_number,vehicle_number:card.vehicle_number, action, message:  `Invalid ${logAction} status`  });
        return client.publish(`${topic}/response/${embed_id}`, JSON.stringify({ success: false, message: `Invalid ${logAction} status` }));
    }

    await TrafficLog.create({ card_id: card.card_id, device_id: device.device_id, action: logAction, time: new Date() });
    client.publish(`${topic}/response/${embed_id}`, JSON.stringify({ success: true, message: `${logAction} logged` }));
